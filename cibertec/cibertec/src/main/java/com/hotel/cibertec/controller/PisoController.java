...
public class PisoController {
    ...
    public ResponseEntity<ApiResponse<?>> registrar(@Valid @RequestBody PisoDto dto) {
        try { return ResponseEntity.ok(ApiResponse.success(service.guardar(dto))); }
        catch (Exception e) { return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage())); }
    }

    @PutMapping("/actualizar/{id}")
    public ResponseEntity<ApiResponse<?>> actualizar(@PathVariable Integer id, @Valid @RequestBody PisoDto dto) {
